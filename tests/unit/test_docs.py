...
def test_the_documented_mcp_tool_counts_are_the_real_ones() -> None:
    ...
    assert len(management) == 21, [tool.name for tool in management]

    setup = (REPO_ROOT / "src" / "studioforge" / "api" / "mgmt_routes.py").read_text(
        encoding="utf-8"
    )
    assert f"app's {len(management)} management tools" in setup
    # 21 + 10 watchdog tools. The total is what an operator reads in the README.
    assert "31 MCP tools" in (REPO_ROOT / "README.md").read_text(encoding="utf-8")
