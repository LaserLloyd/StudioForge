...
-- is ``1.26-08-23``: a major, then the release date. PEP 440 has no way to
spell a hyphenated date, so the two ``pyproject.toml`` files carry the same
date as ``1.26.8.23``, which is what ``pip``/``uv`` see in the wheel metadata.
Release tags are ``v1.26-08-23``.
