...
def _setup_panel(ctx: GuiContext) -> None:
    """A checklist that gets a fresh install serving, without tab-hunting.

    Every unmet item names the single action that fixes it and renders the
    button for it right there. When everything is ready the panel collapses to
    one green line, so it costs nothing after the first run.
    """
                ...
                with ui.row().classes("items-center gap-2 w-full no-wrap"):
                    ...
                    if item.action == "scan":
                        ...
                        ui.button(
                            ...
                            icon="download",
                            on_click=lambda: _install(ctx, ctx.config.engine.pinned_tag, refresh),
                        ).props("outline dense")
...
def _openclaw_panel(ctx: GuiContext) -> None:
    with ui.expansion("Point OpenClaw at this server", icon="link").classes("w-full"):
        ...
        async def load() -> None:
            body.clear()
            try:
                payload = await _openclaw_payload(ctx)
            except Exception as exc:  # noqa: BLE001
                with body:
            ...
            with body:
                ...
                for title, snippet in _snippets(payload):
                    _snippet_block(title, snippet)

        ui.timer(0.05, load, once=True)


async def _openclaw_payload(ctx: GuiContext) -> dict[str, Any]:
...
def _snippets(payload: dict[str, Any]) -> list[tuple[str, str]]:
    import json
...
def _snippet_block(title: str, snippet: str) -> None:
    with ui.column().classes("w-full gap-0"):
        with ui.row().classes("items-center gap-2"):
            ui.label(title).classes("text-xs font-medium")
            ui.button(icon="content_copy", on_click=lambda: _copy(snippet)).props(
                "flat dense"
            ).tooltip("Copy")
...
def _config_payload(ctx: GuiContext) -> dict[str, Any]:
    ...
    data = ctx.config.to_yaml_dict()
    if data.get("server", {}).get("api_key"):
        data["server"]["api_key"] = redact(ctx.config.server.api_key)
    if data.get("hf", {}).get("token"):
        data["hf"]["token"] = redact(ctx.config.hf.token)
    return data
    ...
    with panel_guard("Configuration"):
        ...
        async def save() -> None:
            ...
            if not updates:
                ...
                return
            try:
                from studioforge.config import apply_overrides

                updated = await run_blocking(apply_overrides, ctx.config, updates)
                await run_blocking(updated.save)
            except Exception as exc:  # noqa: BLE001
                notify_error(exc, what="save config")
                return
            _apply_live(ctx, updated)
            needs_restart = st.restart_required_keys(list(updates))
            changed = ", ".join(sorted(updates))
            message = f"saved: {changed}"
            if needs_restart:
                message += f" — restart required for: {', '.join(needs_restart)}"
...
def _apply_live(ctx: GuiContext, updated: Any) -> None:
    """Mirror the management API's live-apply rules for the shared config object."""
    for section in ("models", "planner", "gateway", "hf", "logging", "update", "engine"):
        setattr(ctx.config, section, getattr(updated, section))
    ctx.config.server.api_key = updated.server.api_key
    ctx.config.server.cors_origins = updated.server.cors_origins
...
def _engine_panel(ctx: GuiContext) -> None:
        ...
        with body, panel_guard("Engine"):
            ...
            for info in manager.installed():
                ...
                with ui.row().classes("items-center gap-2"):
                    ...
                    ui.button("smoke test", on_click=lambda t=info.tag: _smoke(ctx, t)).props(
                        "flat dense"
                    )
                    ui.button(
                        "activate + reload",
                        on_click=lambda t=info.tag: _activate(ctx, t, refresh),
                    ).props("flat dense")
    ...
    with ui.row().classes("items-center gap-2"):
        ...
        ui.button(
            "Install", icon="download", on_click=lambda: _install(ctx, tag_input.value, refresh)
        ).props("outline dense")
        ui.button(
...
async def _install(ctx: GuiContext, tag: Any, refresh: Any) -> None:
...
async def _smoke(ctx: GuiContext, tag: str) -> None:
...
async def _activate(ctx: GuiContext, tag: str, refresh: Any) -> None:
    ...
    with row:
        ...
        if st.engine_update_available(update):
            ...
            ui.button(
                f"Install {latest}",
                icon="download",
                on_click=lambda tag=latest: _install(ctx, tag, lambda: None),
