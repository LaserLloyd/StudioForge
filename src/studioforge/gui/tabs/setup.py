...
def _secret_line(
    ...
    *,
    ...
    collapsed expansion behind a switch. Whoever can change the box from this
    panel can already rotate the PIN and edit every setting, so showing it on
    request gives away nothing the panel did not already give away; it just
    stops the hunt.

    ``revealable`` is that condition: a remote viewer on an open install
    cannot change the box (D32), and the PIN is exactly the credential that
    would let them, so for that viewer the eye and the copy button are
    withheld the way ``/api/mcp/info`` withholds the PIN from a remote caller.
    """
    ...
    with ui.row().classes("items-center gap-1 no-wrap"):
        ...
        if value and not revealable:
            ui.label("(read it on the box, or set server.api_key)").classes("text-xs opacity-60")
        elif value:
...
def _network_body(ctx: GuiContext, refresh: Any) -> None:
    ...
    # Resolved at render time from the viewer's own peer: the rule that decides
    # whether this viewer may change the box also decides whether it may read
    # the credential that would let it.
    revealable = viewer_may_change_box(ctx)
...
def _openclaw_panel(ctx: GuiContext) -> None:
    ...
    with ui.expansion("Point OpenClaw at this server", icon="link").classes("w-full"):
        ...
        async def load() -> None:
            ...
            # The same D32 rule as the credential lines above: a remote viewer
            # on an open install gets the snippets masked, and Copy copies
            # the masked text -- otherwise the PIN the API withholds from a
            # remote caller would be one click away here.
            trusted = viewer_may_change_box(ctx)
            with body:
                if trusted:
                    ui.switch(
                        "reveal secrets",
                    ...
                    ui.label(
                        "Generated by the same endpoint sfctl and the MCP server use, so these "
                        "are the exact values this instance is serving on. Copy copies the real "
                        "value even while it is masked here."
                    ).classes("text-xs opacity-70")
                else:
                    ui.label(
                        "Secrets are masked for a viewer on another machine while "
                        "server.api_key is not set; read them on the box, or set the key."
                    ).classes("text-xs opacity-70")
                for title, snippet in blocks:
                    masked = st.mask_secrets(snippet, secrets)
                    shown = snippet if trusted and revealed["on"] else masked
                    copied = snippet if trusted else masked
                    with ui.column().classes("w-full gap-0"):
                        with ui.row().classes("items-center gap-2"):
                            ui.label(title).classes("text-xs font-medium")
                            ui.button(
                                icon="content_copy", on_click=lambda text=copied: copy_text(text)
...
async def _set_autostart(ctx: GuiContext, enable: bool, refresh: Any) -> None:
    ...
    with busy(message="Updating the startup entry…"):
        try:
            # A startup entry on the box, with no API route behind it: the
            # panel is its only surface, so the D32 rule is applied here.
            require_local_admin(ctx, "autostart")
            if enable:
