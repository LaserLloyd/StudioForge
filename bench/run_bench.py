...
def run_one(
    gw: Gateway, args: argparse.Namespace, prompt: dict[str, Any], run: int
) -> dict[str, Any]:
    ...
    print(
        ...
        f"  wall={row['wall_s']}s  peak GiB={peak_gib}{suspect}"
    )
    return row
...
def print_summary(summary: dict[str, Any]) -> None:
    ...
    for key in sorted(summary, key=int):
        ...
        flags = (" CACHE?" if e.get("cache_hit_suspected_runs") else "") + (
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload["validity"] = {
        "valid": len(decisive) == 2 and all(c["valid"] for c in decisive),
        "checks": checks,
    }
    ...
    if not payload["validity"]["valid"]:
        print(
            ...
            "target devices during the runs (see validity.checks) ***"
        )
