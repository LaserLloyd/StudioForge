...
class Gateway:
    def __init__(self, base: str, api_key: str | None, label: str | None) -> None:
        ...
        self.client = httpx.Client(base_url=self.base, headers=headers, timeout=httpx.Timeout(120.0, connect=10.0))
    ...
    def new_client(self) -> httpx.Client:
        return httpx.Client(base_url=self.base, headers=self.headers, timeout=httpx.Timeout(30.0, connect=10.0))
...
def git_info(repo_root: Path) -> dict[str, Any]:
    try:
        ...
        dirty = subprocess.run(["git", "status", "--porcelain"], cwd=repo_root, capture_output=True, text=True, check=True).stdout.strip()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--prompt-lengths", default=DEFAULT_LENGTHS, help="tokens; each clamped to ctx - max-tokens - 64")
    ...
    p.add_argument("--smoke", action="store_true", help="resident instance only: 1024 tokens, 2 runs, 64 tokens, no load, no restore")
    p.add_argument("--no-restore", action="store_true", help="leave the bench instance up; do not reload what was resident before")
    p.add_argument("--api-key", default=os.environ.get("SF_API_KEY") or None, help="bearer key (default: env SF_API_KEY)")
    p.add_argument("--label", default=None, help="suffix for the results file name")
    p.add_argument("--dry-run", action="store_true", help="print the plan and the requests; contact nothing")
...
def preflight(gw: Gateway, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if busy.get("active_requests"):
        problems.append(f"busy.active_requests={busy['active_requests']} ({busy.get('busy_models')})")
    if busy.get("loading"):
...
def validity_check(gw: Gateway, devices: list[int], own_pids: list[int], phase: str) -> dict[str, Any]:
    ...
    print(f"[validity/{phase}] {flag}: {len(result['own'])} own, {len(result['offending'])} foreign compute holder(s) on {devices}; desktop processes collapsed: {result.get('desktop_processes_count')}")
    for h in result["offending"]:
        print(f"    offending pid={h['pid']} {h['name']} class={h['classification']} alias={h['alias']} bytes={h['used_bytes']} per_gpu={h['per_gpu_bytes']} via={h['attribution']}")
    return result


def load_model(gw: Gateway, args: argparse.Namespace, resident: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if resident is not None:
        ...
        if have[:3] == wanted:
            print(f"[load] already resident at the requested shape (priority {resident.get('priority')}); reusing, no reload")
            return {"performed": False, "reused_resident": True, "wall_s": None, "previous_instance": resident}
        print(f"[load] resident at a different shape {have} (priority {resident.get('priority')}); unloading it first (no force is ever passed)")
        gw.post(model_path(args.model, "/unload"), {})
    body = {"ctx_size": args.ctx, "parallel": args.parallel, "devices": args.device_list, "priority": bs.TIER_AGENT}
    ...
    return {"performed": True, "reused_resident": False, "wall_s": round(wall, 2), "previous_instance": resident, "instance": instance}
...
def count_tokens(gw: Gateway, model: str, text: str) -> tuple[int, str]:
    ...
    response = gw.client.post("/v1/tokenize", json={"model": model, "content": text}, timeout=httpx.Timeout(600.0, connect=10.0))
    if response.status_code == 200:
        return len(response.json().get("tokens") or []), "tokenize"
    if response.status_code not in (404, 405):
        raise Refused(f"POST /v1/tokenize -> {error_text(response)}")
    probe = gw.post("/v1/completions", {"model": model, "prompt": text, "max_tokens": 1, "temperature": 0, "cache_prompt": False, "priority": bs.TIER_AGENT}, timeout=600.0)
    timings = probe.get("timings") or {}
    n = int(timings.get("prompt_n") or (probe.get("usage") or {}).get("prompt_tokens") or 0)
    if n <= 0:
        raise Refused("cannot count tokens: no /v1/tokenize route and the probe completion reported no prompt_n")
...
def fit_prompts(gw: Gateway, args: argparse.Namespace, ctx: int) -> dict[int, dict[str, Any]]:
    ...
    print(f"[prompt] calibration via {method}: {sample_tokens} tokens for {sample_words} words -> {ratio:.3f} tok/word")
    ...
    for requested in args.length_list:
        ...
        if tokens > clamp:
            raise Refused(f"prompt for {requested} fitted to {tokens} tokens, above the clamp {clamp}; refusing to exceed ctx")
        prompts[requested] = {"requested": requested, "clamp": clamp, "target": target, "words": words, "tokens": tokens, "text": text, "method": method}
...
def run_one(gw: Gateway, args: argparse.Namespace, prompt: dict[str, Any], run: int, own_pids: list[int] | None = None) -> dict[str, Any]:
    ...
    payload = {
        ...
        "temperature": 0, "top_k": 1, "seed": args.seed, "cache_prompt": False, "ignore_eos": True,
    ...
    try:
        with gw.client.stream("POST", "/v1/completions", json=payload, timeout=httpx.Timeout(RUN_TIMEOUT_S, connect=10.0)) as response:
            if response.status_code >= 400:
                response.read()
                raise Refused(f"POST /v1/completions (run {run}, length {prompt['requested']}) -> {error_text(response)}")
            for line in response.iter_lines():
                ...
                if chunk.get("error"):
                    raise Refused(f"in-stream error (run {run}, length {prompt['requested']}): {json.dumps(chunk['error'])}")
                if ttft is None and any((c.get("text") or (c.get("delta") or {}).get("content")) for c in chunk.get("choices") or []):
    ...
    if timings is None:
        ...
        row.update({"timings_missing": True, "prompt_n": (usage or {}).get("prompt_tokens"), "predicted_n": completion_tokens or None,
                    ...
                    "decode_tps": round(completion_tokens / (wall - (ttft or 0)), 2) if completion_tokens and wall > (ttft or 0) else None})
    else:
        row.update({"timings_missing": False, "prompt_n": timings.get("prompt_n"), "cache_n": timings.get("cache_n"), "predicted_n": timings.get("predicted_n"),
                    "prefill_tps": timings.get("prompt_per_second"), "decode_tps": timings.get("predicted_per_second"), "timings": timings})
...
def restore(gw: Gateway, args: argparse.Namespace, residents: list[dict[str, Any]], load_record: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if load_record.get("performed"):
        ...
        actions.append({"action": "unload", "model_id": args.model, "result": gw.post(model_path(args.model, "/unload"), {})})
    for model_id, row in previous.items():  # includes the model under test if it was resident before
        plan = row.get("plan") or {}
        current = instance_of(gw, model_id)
        if current is not None and bs.plan_tuple(current.get("plan")) == bs.plan_tuple(plan):
            actions.append({"action": "skip", "model_id": model_id, "reason": "already resident at the previous shape"})
            continue
        body = {"ctx_size": plan.get("ctx_size"), "parallel": plan.get("parallel"), "devices": plan.get("devices"),
                "kv_cache_type": plan.get("kv_cache_type"), "kv_cache_type_v": plan.get("kv_cache_type_v"), "priority": row.get("priority")}
        ...
        try:
            ...
            actions.append({"action": "load", "model_id": model_id, "body": body, "ok": False, "error": str(exc)})
    return actions
...
def write_results(results_dir: Path, sha: str, args: argparse.Namespace, payload: dict[str, Any]) -> Path:
...
def print_plan(args: argparse.Namespace) -> None:
    ...
    print(f"config: ctx {args.ctx}, devices {args.device_list}, parallel {args.parallel}, priority {bs.TIER_AGENT} (agent tier), max_tokens {args.max_tokens}, runs {args.runs} (run 0 discarded), seed {args.seed}, smoke {args.smoke}")
    for requested in args.length_list:
        clamp = bs.clamp_prompt_length(requested, args.ctx, args.max_tokens)
        print(f"  prompt {requested}: clamp {clamp}, target {int(clamp * (1 - TOKEN_TOLERANCE))} tokens, ~{bs.estimate_words(clamp, 1.3)} words at the 1.3 tok/word estimate (calibrated live via POST /v1/tokenize)")
    steps = ["GET /api/health (ready, busy.active_requests == 0, no load in progress)", "GET /api/status (leases, benchmark, queue, resident instances snapshot)", "GET /api/models (model in registry)",
             "GET /api/vram/holders + GET /api/gpus (validity, pre-load)"]
    if not args.smoke:
        steps += [f"POST {model_path(args.model, '/unload')} (only if resident at a different shape)", f"POST {lp} {json.dumps({'ctx_size': args.ctx, 'parallel': args.parallel, 'devices': args.device_list, 'priority': bs.TIER_AGENT})} (timed: load wall)"]
    steps += [f"GET {model_path(args.model, '/introspect')} + GET /api/status (effective launch, plan, engine tag)", "GET /api/vram/holders (validity, post-load)",
    ...
    if not args.smoke and not args.no_restore:
        steps += [f"POST {model_path(args.model, '/unload')}", "POST /api/models/{id}/load for each previously resident model (its plan tuple + priority)"]
    steps.append(f"write {Path(args.results_dir) / bs.result_filename('<sha>', args.label, args.smoke)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {"schema": 1, "smoke": args.smoke, "label": args.label, "git": git, "timestamp": started_at.isoformat(), "argv": sys.argv[1:]}
    ...
    try:
        ...
        payload["server"] = {"url": args.server, "version": pre["health"].get("version"), "engine": (pre["status"].get("engine") or {}).get("tag")}
        ...
        ctx_per_slot = int(((instance.get("effective") or {}).get("ctx_per_slot")) or (instance.get("plan") or {}).get("ctx_size") or args.ctx)
        payload["config"] = {"requested": {"model": args.model, "ctx_size": args.ctx, "parallel": args.parallel, "devices": args.device_list, "priority": bs.TIER_AGENT, "max_tokens": args.max_tokens, "runs": args.runs, "prompt_lengths": args.length_list, "seed": args.seed},
                             "effective": instance.get("effective"), "plan": instance.get("plan"), "launch_args": instance.get("launch_args"), "instance_priority": instance.get("priority"),
                             "resolved_engine_tag": instance.get("resolved_engine_tag"), "introspect_actual": introspect.get("actual"), "ctx_per_slot": ctx_per_slot}
        payload["load"] = {k: v for k, v in load_record.items() if k != "instance"}
        checks.append(validity_check(gw, args.device_list, [instance["pid"]] if instance.get("pid") else [], "post-load"))
