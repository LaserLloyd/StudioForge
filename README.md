...
**Status:** v1.26-08-28. Windows is the reference platform and runs it daily; Linux is supported (CI
...
version lives in `src/studioforge/__init__.py` — `1.26-08-28` — and is what `GET /api/version`,
`GET /health`, the MCP `server_status` tool and `sfctl status` report. PEP 440 has no way to spell
a hyphenated date, so both `pyproject.toml` files carry the same date as `1.26.8.28`, which is what
`pip`/`uv` see in the wheel metadata; the `sfctl` companion ships from the same release and carries
the same version, and release tags are `v1.26-08-28`. Check what a server is actually running with
