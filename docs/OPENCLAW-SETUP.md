...
| Agent falls back to its secondary provider whenever the model is not loaded | not the missing pin — every model loads on demand. Almost always a saved `device_override` naming a card that is leased (ComfyUI's), so every on-demand load is refused `507 gpu_leased` | `plan_load(model_id)` shows the refusal; `sfctl models settings <id> --set device_override=null` (save `allowed_devices` if the model must stay on certain cards). See OPENCLAW.md, *On-demand models* |
